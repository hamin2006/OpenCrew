...
class TestFfmpegIsNotResolvedFromPath:
    ...
    @staticmethod
    def _plant(directory, name="ffmpeg"):
    ...
    def test_a_trusted_directory_wins_over_a_writable_candidate(self, tmp_path, monkeypatch):
        ...
        assert transcribe._find_ffmpeg() == str(system)
    ...
    def test_a_fixed_candidate_is_still_used_when_no_system_copy_exists(
        ...
        monkeypatch.setattr(transcribe.platform_compat, "trusted_system_path", lambda: None)
        assert transcribe._find_ffmpeg() == str(candidate)

    def test_the_candidate_order_puts_writable_dirs_last(self):
        """Pinned as an ORDER, not a set: the security property is the precedence."""
        dirs = transcribe._ffmpeg_candidate_dirs()
        home = os.path.expanduser("~")
        user_owned = [i for i, d in enumerate(dirs) if d.startswith(home)]
        packaged = [i for i, d in enumerate(dirs) if not d.startswith(home)]
        assert packaged and user_owned, dirs
        assert max(packaged) < min(user_owned), f"a writable dir can shadow a packaged one: {dirs}"
