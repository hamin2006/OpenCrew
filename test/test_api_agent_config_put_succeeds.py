...
async def test_agent_config_write_holds_the_mcp_transaction_lock(tmp_path):
    ...
    assert all(
        held for _p, held, _f in spec_writes
...
async def test_agent_config_write_goes_through_one_shielded_offload(tmp_path):
    ...
    assert {label for label, _ in writes} == {
        "bookkeeping",
        "mc_cfg",
        "spec",
...
@pytest.mark.asyncio
async def test_agent_config_put_persists_nothing_when_cancelled_waiting_for_the_lock(tmp_path):
    """A PUT cancelled while WAITING for the lock must leave no durable trace.

    The bookkeeping writes and the spec write are one unit: ``removedTools`` in
    config.json and the lifted model state in the agent_model_state.json sidecar
    describe the very spec being installed. Taking the lock only around the spec
    write left both of them OUTSIDE it, so a PUT that queued behind another
    holder of the MCP transaction lock and was then cancelled (client disconnect,
    gateway shutdown) had already persisted the bookkeeping for a spec that never
    landed — the tear this covers. One lock spanning from the first durable write
    to the last makes the half-applied state unrepresentable, because a
    cancellation at the lock wait happens before anything is written at all.
    """
    import contextlib
    ...
    @contextlib.asynccontextmanager
    async def _lock_held_by_someone_else():
        # Blocks in __aenter__, exactly as the real lock does while another
        # transaction holds it — so the cancellation lands at the lock WAIT.
        waiting.set()
    ...
    assert writes == [], (
        "a PUT cancelled at the lock wait had already persisted a config write: "
        f"{writes} — the bookkeeping is durable while the spec it describes is not"
    )
    assert lifted == [], (
        "a PUT cancelled at the lock wait had already lifted bookkeeping into the "
        "agent_model_state sidecar for a spec that was never installed"
    )
