...
def test_the_staging_file_is_per_process(monkeypatch, tmp_path):
    """Two processes downloading the same model must not share one staging path.

    `ModelStore` serialises callers inside one process, but a gateway, an MCP server
    and a CLI invocation are three processes over one data home. Sharing
    ``ggml-base.bin.part`` let their writes interleave into it, so the sha256 pin
    failed BOTH transfers: a model downloading fine twice completed zero times.
    """
    payload = b"weights" * 100
    model = _pin(monkeypatch, tmp_path, payload)
    seen: list[str] = []
    ...
    with pytest.raises(models.ModelDownloadError):
        models._download_blocking(model)
    assert seen, "no staging file was observed mid-transfer"
    assert all(str(os.getpid()) in name for name in seen), seen
