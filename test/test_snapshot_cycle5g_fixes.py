...
class TestComponentJsonIsValidatedBeforeInstall:
    ...
    def test_merge_skips_a_crons_file_of_the_wrong_shape_rather_than_installing_it(
        ...
        `_merge_crons` skips an unusable file and continues. The pre-flight still stands
        aside for a wrong shape (it is a valid JSON object, so `_refuse_unless_json_object`
        passes), because the reader's own guard now covers it. The property -- a bad entry
        is never handed to the reader -- is unchanged; where it is enforced moved.
        """
        # A valid JSON object of the wrong shape is NOT refused by the pre-flight...
        payload = tmp_path / "kirocrew-snapshot-20260101T000000Z"
        ...
        # ...it is the reader's shape guard that rejects it before any field access.
