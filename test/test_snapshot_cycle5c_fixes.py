...
class TestAFailedTreeReplacementPutsTheTreeBack:
    ...
    def test_a_target_the_restore_created_is_removed_by_recovery(
        ...
        rc = snap.restore_main(
            [str(bundle), "--mode", "replace", "--force", "--components", "workspace,skills"]
        )
        assert rc == 1
        out = capsys.readouterr().out
        assert "Restoring the previous state" in out, out
...
class TestAnUnrestorableBundleIsNeverUploaded:
    def test_a_bundle_over_the_download_bound_is_not_published(
        ...
        monkeypatch.setattr(snap, "_resolve_aws_profile", lambda p: ("someprofile", "us-west-2"))
        monkeypatch.setattr(
            snapshot_remote, "upload", lambda *a, **k: uploaded.append(a) or "s3://x/y"
        )
        monkeypatch.setattr(snapshot_remote, "load_destination", lambda *a, **k: _FakeDest())
