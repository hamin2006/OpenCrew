...
| Injected-message envelope prefixes | `dashboard/state.py` (re-exports) / `constants.py` | `CRON_NOTIFY_PREFIX` / `SUBAGENT_COMPLETION_PREFIX` / `SUBAGENT_BATCH_COMPLETION_PREFIX` live in `constants.py` so platform/core can import them without the dashboard layer. `dashboard/state.py` re-exports them next to `CRON_NOTIFY_END` / `CRON_NOTIFY_RE`, `SUBAGENT_SYNTHESIS_PREFIX`, and the five `*_RECOVERY_PREFIX` markers so the frontend still has one list to mirror. See [injected-messages](injected-messages.md). |
...
| Cross-cutting shared constants | `constants.py` | `KIROCREW_SPAWNED_ENV`, `ENV_TRUTHY`, `CHAT_TURN_TIMEOUT`, `COMPACT_WAIT_TIMEOUT_SECS` (one budget, shared by manual and automatic compaction), the `[OPTIONS:]` parse regexes, `CRON_NOTIFY_PREFIX` / `SUBAGENT_COMPLETION_PREFIX` / `SUBAGENT_BATCH_COMPLETION_PREFIX` (injected openers core/platform must import), `DATA_WARNING`, `BANNER`. |
...
| AgentCore Gateway MCP name + inbound sidecar dir | `platform/agentcore_gateway.py` | `GATEWAY_SERVER_NAME` (`agentcore-gateway`), `INBOUND_DIR_NAME` (`agentcore-inbound`), `UNATTENDED_SESSION_PREFIXES`, companion `status()` keys (`authorizationUrl`, `credentialKind`, `vaultedOwnerToken`). |
