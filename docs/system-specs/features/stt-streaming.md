...
- `{"type":"ready"}`: the session is live, the client may send audio.
...
`Endpointer` for the next one. It does **not** end the session, and
...
captive-portal HTML body can only fail verification. A file already on disk is
trusted by provenance and is not re-hashed on load.
