...
- **Plane A — the host gate** (`HookManager.on_tool_call`, the primary
  ...
  form is lossy when either segment contains `__`; a raw title with multiple
  separators is therefore not parsed without an exact server roster, and spec
  adapters cache the unique roster-resolved pair before the permission frame.
