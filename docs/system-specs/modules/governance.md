...
writable.** Each component is tested twice — against the mode bits, and against the kernel's own answer
...
however it got onto disk — on the boot path and in a cache-only child alike. That is the
control to reach for on a host that cannot be sandboxed.
...
it starts the poller. Hooks run on the refresher thread after the install, best-effort: an
exception is logged and the next hook still runs, because a control that cannot be re-derived
must not stop a ceiling that WAS installed from being reported as installed. The revocation
