...
allowlist (§6) forwards only the peer's `api/chat` surface, so the peer's own
token-minting routes are unreachable through it. The transfer needs the
