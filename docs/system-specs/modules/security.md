...
directory, because blocking it would hide `config.toml`. When `CODEX_HOME` is set,
the sensitive-target builder re-anchors `auth.json` under that effective root and
includes the resolved override in its cache key; filesystem and normalized shell
checks therefore move with the credential while `config.toml` remains readable. The list is written
...
directory; a project spec reusing that basename is not.
