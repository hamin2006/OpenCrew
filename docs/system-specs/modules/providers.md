...
admits only Kiro CLI, KAS, Codex, and Claude. Goose, OpenCode, pi, and
registry-only adapters remain described and discoverable but cannot be persisted
until basic validation evidence admits them to the set. KAS is fully described
and is selectable (cli-fronted via `kiro-cli`). Mid-turn steer is measured and
granted (`ACP_BACKENDS_STEER`); session sharing stays fail-closed until keep-aware
teardown lands. Spec-adapter steer degrades to follow-up.
...
the session exists. Codex is selectable, but startup remains fail-closed unless
the operator uses the named ungated-tools opt-out because `mode=read-only` is a
post-session config acknowledgement. Adapter resolution honours Windows
...
- A dashboard change to `agent.acp_backend` refreshes the captured provider
  ...
  A newly discovered adapter can therefore appear as described but withheld
  without becoming a valid config value. Backend switches
