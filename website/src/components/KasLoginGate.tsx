  ...
  return (
    ...
    >
      ...
      <div className="mt-7 flex w-full max-w-md flex-col gap-3">
        ...
        />
        <ProviderButton
          icon={<Cloud className="lucide-inline" />}
      ...
      {beginError ? (
        <p className="mt-4 max-w-md text-[13px] leading-relaxed text-danger" role="alert">
          {i18nT('components.kasLogin.could_not_start_sign_in')} {beginError}
        </p>
      ) : null}
...
// The loopback counterpart of the device wait: the gateway shares a machine
// with the browser, so the callback returns on its own and there is no code to
// show — just a lighter holding state while the status poll observes it land.
function LoopbackWaiting({
  ...
  return (
    ...
    >
      ...
      <h1 className="mt-2 text-3xl font-bold tracking-tight text-text-strong">
        {i18nT('components.kasLogin.waiting_for_your_browser')}
      </h1>
      <p className="mt-3 max-w-lg text-sm leading-relaxed text-muted">
        {i18nT('components.kasLogin.loopback_hint')}
      </p>
      <p className="mt-6 flex items-center gap-2 text-[13px] text-muted" aria-live="polite">
        <Loader2 className="lucide-inline animate-spin text-accent" />
        {i18nT('components.kasLogin.waiting_for_you_to_approve')}
      ...
      <div className="mt-6">
        <Btn type="button" onClick={onCancel}>
          {i18nT('components.kasLogin.cancel')}
        </Btn>
      </div>
    </GateShell>
  )
...
export default function KasLoginGate({ children }: { children?: ReactNode }) {
  ...
  >(null)
  // Loopback sign-in in flight: the browser was asked to open, and the status
  // poll below is what observes the callback landing.
  const [loopbackProvider, setLoopbackProvider] = useState<KasLoginProvider | null>(null)

  const statusQuery = useQuery({
    queryKey: QUERY_KEY,
    queryFn: api.kasLoginStatus,
    // While a loopback sign-in is pending the status endpoint is the only
    // signal the callback landed, so poll it; otherwise a slow background read
    // is plenty.
    refetchInterval: loopbackProvider ? 3_000 : 30_000,
  })
  ...
  // Loopback wait resolves itself: once the status poll reports authenticated,
  // clear the holding state so a later sign-out returns to a clean chooser.
  const authenticated = !!status?.authenticated
  useEffect(() => {
    if (authenticated) setLoopbackProvider(null)
  }, [authenticated])
  ...
  if (loopbackProvider) {
    return (
      <LoopbackWaiting provider={loopbackProvider} onCancel={() => setLoopbackProvider(null)} />
    )
  }
  ...
  return (
    <Chooser
      ...
      onPick={(provider) => {
        beginMutation.reset()
        if (status.transport === 'device') beginMutation.mutate(provider)
        else setLoopbackProvider(provider)
      }}
    />
  )
}
