 ...
 *   `announceAppsChanged` so the Updates list drops rows mid-batch.
...
describe('useAppUpdates — Update All', () => {
  ...
  it('runs sequentially with {done,total} progress and announces each success', async () => {
    const deferred = deferUpdates()
    ...
    // Each success announces AS IT LANDS, not once at the end.
    expect(announce).toHaveBeenCalledTimes(1)
