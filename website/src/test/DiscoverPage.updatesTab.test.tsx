...
describe('Updates sub-page — list rendering', () => {
  ...
  it('per-row pending state disables ONLY the updating row', async () => {
    const deferred = deferUpdates()
    renderDiscover('/apps/-/updates')

    fireEvent.click(rowUpdateButtons()[0])
    await waitFor(() => expect(rowUpdateButtons()[0]).toBeDisabled())
    // The sibling row and Update All stay live — a single in-place update
    // freezes its own row, not the worklist.
    expect(rowUpdateButtons()[1]).not.toBeDisabled()
    ...
    await waitFor(() => expect(rowUpdateButtons()[0]).not.toBeDisabled())
