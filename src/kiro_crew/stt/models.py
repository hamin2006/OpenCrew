...
``ModelStore._verified_on_disk``), because provenance is not enough here: nothing
fences this directory, so a same-size file dropped over the weights would
otherwise be trusted forever. Not cached against the file's metadata either --
``os.utime`` is available to anything that can write the file, so a size-and-mtime
memo is forgeable by the same actor it is meant to catch. That is a deliberate divergence from
...
def _download_blocking(
    ...
    # Staging name is per-PROCESS, not a fixed ``.bin.part``. `ModelStore` serialises
    # concurrent callers inside one process, but nothing serialises across them: a
    # gateway, an MCP server and a `kirocrew` CLI invocation each run their own store
    # and all three resolve the same data home. Sharing one staging path let two
    # transfers interleave their writes into it -- the sha256 pin then failed BOTH,
    # so a model that was downloading fine twice ended up downloading successfully
    # zero times, and one process's rename could move a file the other was still
    # writing.
    staging = target.with_name(f"{target.name}.{os.getpid()}{_STAGING_SUFFIX}")
