...
class SubagentManager:
    ...
    async def _safe_announce(self, info: SubagentInfo) -> None:
        ...
        if info.batch_id and not info._coordinator_admitted:
            await self._report_synthetic_batch_terminal(info)
            return
        try:
    ...
    async def _shadow_submit_accepted_run_unchecked(
        ...
        *,
        ...
        try:
            ...
            # An injected coordinator may report a timeout after its durable
            # write committed. Treating that as a definite missing fence would
            # emit one local failure and let recovery emit a second
            # interruption. Read the stable command once: an owned, unexpired
            # committed claim is safe to resume; every other result stays
            # exclusively recoverable.
            if generation is not None and generation != info._coordinator_shadow_generation:
                return True
    ...
    def _readmit_unsettled_shadow_submissions(
        self,
        ...
        for owner in owners:
            ...
            seen.add(owner.id)
            try:
                if clear_tombstone_for_recovery(owner.id):
                    logger.warning(
        ...
        try:
            ...
            for report_task in self._lifecycle.pending_reports():
                ...
                if owner is not None and not owner._reported_to_parent:
                    owners.append(owner)
            self._readmit_unsettled_shadow_submissions(owners)
            raise
