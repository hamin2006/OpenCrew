...
def _fts_layout(conn: "sqlite3.Connection") -> tuple[list[str], set[str]]:
    ...
    for name, sql in conn.execute(
        ...
        if sql and "USING fts" in sql.replace("using fts", "USING fts"):
            ...
            if re.search(r"content\s*=\s*(''|\"\")", sql):
...
def _paged_rows(
    ...
    """Yield every row of *table* a page at a time, ordered by *handle*.
...
def _refuse_update_triggers_that_destroy_rows(
    ...
    The residual, stated rather than implied: this reads the trigger BODY, so a trigger that
    destroys a row by other means -- `INSERT OR REPLACE` over an existing key -- is not
    caught. Bounding that needs the effects, not the text, which is separate work.
    """
    suspects: list[str] = []
    ...
    if suspects:
        raise _TableNotInspectable(
            f"{rel} has UPDATE trigger(s) that DELETE rows and are not full-text index "
