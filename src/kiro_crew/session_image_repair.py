...
def _generic_image_blocks(node: Any) -> Iterator[dict]:
    ...
    if isinstance(node, dict):
        ...
        for value in node.values():
            yield from _generic_image_blocks(value)
        ...
        with path.open("rb") as handle:
            for line_no, raw_line in enumerate(handle):
                ...
                try:
                    ...
                    # Not decodable, so not the JSON kiro-cli writes, so not a
                    # record carrying media. Passed through byte-for-byte.
                    out_lines.append(stripped)
                    continue
                ...
                unanchored = [
                    b
                    for b in _generic_image_blocks(record)
                    if id(b) not in anchored_ids and _block_bytes(b) is not None
                ]
                if unanchored:
