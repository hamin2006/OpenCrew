...
Kiro CLI, KAS, Codex, and Claude; registry discovery and the other built-in
descriptors do not widen that reviewed set. Codex and Claude are gated before
the first prompt. The withheld goose path uses `approve` rather than its default
`auto` (auto-approve);
...
> `pi-acp` binary (not `pi acp`). Only Kiro CLI, KAS, Codex, and Claude are
...
- **Codex** is gated by ACP v1 session config `mode=read-only`, applied after
  ...
  advertise that value or the write fails. Because that route is also armed
  after session creation, Crew MCP is withheld from Codex sessions too.
