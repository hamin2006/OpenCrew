    ...
    async def run(self) -> None:
        ...
        if not self._test_mode:
            with contextlib.suppress(Exception):
                ...
                # The hook is registered BEFORE the poller starts, so the first installed
                # ceiling already re-derives what was materialised from the previous one.
                # Most governed controls are live evaluations and need nothing here; a
                # published tailnet origin is the exception, because its gate fires when
                # publish is CALLED and so does not retract what is already serving.
