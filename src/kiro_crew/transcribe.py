...
def _ffmpeg_candidate_dirs() -> list[str]:
    ...
    Ordered MOST-TRUSTED FIRST, which is the whole point of the ordering: whatever
    this resolves is exec'd by the gateway, so a directory the agent can write must
    never be able to SHADOW one it cannot. Package-manager prefixes therefore come
    before the per-user extraction dirs, and `_find_ffmpeg` consults
    `platform_compat.trusted_system_bin` ahead of this list entirely.

    The two per-user dirs stay last rather than being dropped: nothing else would find
    an ffmpeg a user unzipped themselves, and refusing it would make voice memos
    undecodable on a host with no packaged ffmpeg. They are agent-writable, so a host
    with no system ffmpeg at all still trusts them -- searching them last is what
    bounds that to "the only ffmpeg present" instead of "the one that wins".
    ...
    dirs.extend([os.path.expanduser("~/ffmpeg"), os.path.expanduser("~/.local/bin")])
    return dirs
