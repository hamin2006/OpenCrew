...
class _NoRedirects(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):  # noqa: D102
        ...
        scheme = urllib.parse.urlsplit(newurl).scheme or "unknown"
...
def _opener_handlers(url: str) -> "list[urllib.request.BaseHandler]":
    ...
    if _is_loopback(urllib.parse.urlsplit(url).hostname or ""):
...
def _cached_max_cache_age(body: bytes) -> int:
    ...
    if isinstance(declared, bool) or not isinstance(declared, (int, float)):
        return 0
    if not math.isfinite(declared) or declared != int(declared) or declared <= 0:
        return 0
    return int(declared)
...
def _load_by_fetch(dist: PolicyDistribution, cached: Optional[CachedPolicy]) -> _LoadOutcome:
    ...
    if fetched.not_modified and cached is not None:
        ...
        if not write_cache(
            body, source=dist.source, etag=fetched.etag, last_modified=fetched.last_modified
        ):
...
def refresh_now(*, force: bool = False) -> RefreshOutcome:
    ...
    if fetched.not_modified:
        ...
        # Nothing to install when there is no cache, or when the cache is already what
        # is running. An EMPTY digest deliberately does NOT skip: it means this process
        # does not know what it installed — the case of a boot that DEGRADED to
        # ungoverned — and skipping there would leave such a host permanently below a
        # ceiling another process had already cached. Tier-1 precedence is guarded
        # separately, above, by asking the loader ladder's own question.
        if cached is None or _body_digest(cached.body) == installed:
            if cached is not None:
                # Re-validate even when nothing changed. "Unchanged" is a statement
                # about the DOCUMENT, and the trust root is a separate input that moves
                # on its own schedule: a fleet turning on ``require_policy_signature``
                # or rotating a trust key makes the running ceiling untrusted without
                # the endpoint publishing anything, and a 304 would otherwise let it
                # stand indefinitely. Cheap enough to do per interval.
                try:
    ...
    _record_installed(body)
    # After the install, so a hook sees the ceiling it is meant to enforce.
    _run_post_install_hooks()
    logger.info(
        "installed a refreshed governance ceiling from the %s source",
        urllib.parse.urlsplit(dist.source).scheme or "unknown",
    )
...
def _audit_refresh(status: str, dist: PolicyDistribution) -> None:
    ...
    **Nothing here names the endpoint.**  The source is recorded as its SCHEME, and
    ...
    try:
        ...
        scheme = urllib.parse.urlsplit(dist.source).scheme or "unknown"
...
def _run_post_install_hooks() -> None:
    ...
    for hook in list(_POST_INSTALL_HOOKS):
        try:
            ...
            logger.warning(
                "a post-install governance hook failed; a control materialised from the "
                "previous ceiling may not have been re-derived",
                exc_info=True,
            )
...
def distribution_posture() -> Dict[str, object]:
    ...
    posture.update(
        {
            "configured": True,
            "source_scheme": urllib.parse.urlsplit(dist.source).scheme.lower(),
