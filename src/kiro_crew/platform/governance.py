...
def mcp_title_to_ref(title: str, *, server_names: Collection[str] = ()) -> str:
    ...
    # With one separator the boundary is intrinsically unambiguous. More than
    # one can belong to either segment; without exactly one roster match, do not
    # manufacture a security identity from the lossy title encoding.
    if rest.count("__") == 1:
