...
class TokenStore:
    ...
    def load(self, identity: str) -> KasToken | None:
        ...
        except (OSError, ValueError, KeyError, UnicodeError, InvalidTag) as err:
            # Unreadable store, malformed envelope, tampered/undecryptable entry:
            # treat as absent rather than crashing status with a 500. InvalidTag
            # also covers a ciphertext transplanted between entries (AAD mismatch).
