...
def _upload_bundle(outfile: Path, args: argparse.Namespace, selected: list[str]) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="kirocrew-redact-") as tmpdir:
        workdir = Path(tmpdir)
        # Locked down BEFORE any child is created, and as a DIRECTORY rather than a file.
        # `mkdtemp` gives 0700 on POSIX, but Windows derives access from the DACL and
        # ignores the mode, so a permissive parent DACL is inherited by every child --
        # which on this path means a redacted copy of the operator's memory readable by
        # another local principal. `restrict_dir_to_owner` is the cross-platform form, and
        # doing it before the children exist is what makes it protection rather than a
        # late correction.
        platform_compat.restrict_dir_to_owner(workdir)
        try:
...
def snapshot_main(
    ...
    # Reported last so the local bundle and the prune are both accounted for first: the
    # local backup succeeded even when the destination did not, and the operator needs to
    # know both facts rather than only the failure.
    if upload_rc != 0:
        return upload_rc
    return 0
...
def _do_replace(
    ...
    except (OSError, DatabaseCopyFailed, pinned_fs.PinnedPathRefusal) as e:
        # `PinnedPathRefusal` belongs here and is NOT an OSError. This exposure was created
        # by routing the mutation phase through the pinned primitives: `must_create=True`
        # refuses a root recreated after its own rmtree, and a fatal skip reporter refuses a
        # file it cannot copy whole. Both fire MID-mutation, so leaving them out meant the
        # one class of failure the phase two rollback exists for walked straight past it and
        # left live state half replaced.
        #
...
def _do_merge(
    ...
    if _want(components, "memory"):
        for tree in COMPONENTS["memory"].trees:
            ...
            if sd.is_dir():
                dd = mc / tree
                # `parents=True` creates the nested `workspace/` parent too, so there is no
                # separate parent mkdir here. There WAS one, and it was pure redundancy: a
                # second by-name mutation of the same chain that the line below already
                # covers. Review flagged it as by-name path reuse and was right to -- the
                # correct answer was to delete it, not to guard it.
