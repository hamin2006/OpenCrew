...
def _voice_extra_pip_command() -> str:
    """The command that installs the optional ``voice`` extra into THIS interpreter.

    It has to be the gateway's own interpreter: both extras it carries (the
    recogniser for ``local``, ``boto3`` + ``amazon-transcribe`` for
    ``transcribe``) are imported in-process, so a system python or a ``--user``
    install elsewhere would not be importable here.
    """
    if os.name == "nt":
        # The user's shell is unknowable here (they may paste this into
        # PowerShell OR cmd), so the form must be SILENT-CORRUPTION-FREE in
        # both, and PowerShell is the harder shell: a double-quoted string still
        # expands ``$name`` and honours backtick escapes, and so does a bare
        # unquoted token — both are legal path characters, so either form
        # silently rewrites an interpreter under e.g. ``C:\tools\$python\...``
        # into a path that does not exist. Single quotes are PowerShell's LITERAL
        # form (no expansion, no escapes, spaces included), with ``&`` invoking
        # the quoted path, so the interpreter reaches pip byte-for-byte —
        # including the all-users ``C:\Program Files\...`` layout an unquoted
        # form cannot express. cmd performs no ``$`` or backtick processing at
        # all and rejects the leading ``&`` loudly ("... was unexpected"), so a
        # cmd user gets a clear error to re-quote for, never a corrupted install.
        # A literal single quote in the path is escaped by doubling, PowerShell's
        # own rule.
        exe = sys.executable.replace("'", "''")
        return f"& '{exe}' -m pip install kirocrew[voice]"
    return f"{shlex.quote(sys.executable)} -m pip install 'kirocrew[voice]'"
...
def _stt_prereq_commands(provider: str = "local") -> list[str]:
    ...
    if needs_extra and _pip_install_channel_available():
        cmds.append(_voice_extra_pip_command())
