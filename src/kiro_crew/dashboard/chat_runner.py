            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                if outcome == "approved":
                    ...
                    if _pre_tool_hooks_should_block(pre_hook_results):
                        ...
                        #
                        # `cmd` is the command as extracted for the tiers above and
                        # is empty for a structured tool call, so falling back to
                        # the event's own shell command keeps a non-system program
                        # (a pip-installed `aws`) from re-prompting on every use
                        # after the human has already identified it once. It is the
                        # same field the hook tier above already trusts to DECIDE a
                        # refusal, so it is no weaker a source for recording one.
                        _pin_cmd = cmd or (getattr(event, "shell_command", "") or "")
                        if _pin_cmd:
                            await asyncio.to_thread(pin_human_approval, _pin_cmd)
                        await client.approve_tool(event.request_id)
