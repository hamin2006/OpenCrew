// ACP compaction probe v2: build small context, /compact, measure usage.
...
function handle(line) {
  ...
  if (msg.method === "session/update") {
    ...
    if (upd.sessionUpdate === "usage_update") {
      const u = { used: upd.used, size: upd.size, usage: upd.usage, cost: upd.cost };
      usages.push(u);
  ...
  if (msg.error) {
    console.log("===ERROR " + label + "===", JSON.stringify(msg.error).slice(0, 400));
    child.kill();
    process.exit(1);
    return;
  }
  ...
  const texts = Array.isArray(result.content)
    ? result.content.map((c) => c.text || "").join("").slice(0, 80)
    : "";
  console.log("===RESPONSE " + label + "===", JSON.stringify({ text: texts, keys: Object.keys(result) }).slice(0, 160));
  if (label === "initialize") {
    ...
    prompt(3, "Write exactly 60 words about the Roman Empire.");
  } else if (label === "p3") {
    prompt(4, "Write exactly 60 words about the fall of Constantinople.");
  } else if (label === "p4") {
    console.log("===SENDING_COMPACT===");
    prompt(5, "/compact");
  } else if (label === "p5") {
